  ...
  return (
    <Box component="section" sx={{ width: "100%", py: 2, px: { xs: 2, md: 3 } }}>
      ...
      <Grid container spacing={2} alignItems="stretch">
        {promoCards.map((p) => (
          <Grid key={p.id} item xs={12} sm={6} md={6} lg={6}>
            <Card sx={{ position: "relative", overflow: "hidden", borderRadius: 2, border: "1px solid #eee", height: "100%" }}>
              <Grid container>
                <Grid item xs={7} sx={{ p: 3 }}>
                  <Typography variant="overline" sx={{ color: "#8a8a8a" }}>
                    {p.subtitle}
                  </Typography>
                  <Typography variant="h5" sx={{ fontWeight: 800, color: "#ff6b35", mt: 1 }}>
                    {p.title}
                  </Typography>
                  <Typography variant="h3" sx={{ color: "#ef2f06", fontWeight: 800, my: 1 }}>
                    {p.sale}
                  </Typography>
                  <CardActions sx={{ pl: 0 }}>
                    <Button
