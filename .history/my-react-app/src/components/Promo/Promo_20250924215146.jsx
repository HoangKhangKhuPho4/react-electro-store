      ...
      <Grid container spacing={2} alignItems="stretch" sx={{ flexWrap: 'nowrap' }}>
        {promoCards.map((p) => (
          <Grid key={p.id} item xs={6} sm={6} md={6} lg={6} sx={{ flex: '1 1 50%' }}>
            <Card sx={{ position: "relative", overflow: "hidden", borderRadius: 2, border: "1px solid #eee", height: "100%", minHeight: 150 }}>
              <Grid container>
                <Grid item xs={7} sx={{ p: 1.5 }}>
                  <Typography variant="overline" sx={{ color: "#8a8a8a", fontSize: '0.65rem' }}>
                    {p.subtitle}
                  </Typography>
                  <Typography variant="subtitle1" sx={{ fontWeight: 800, color: "#ff6b35", mt: 0.2, fontSize: '1rem' }}>
                    {p.title}
                  </Typography>
                  <Typography variant="h5" sx={{ color: "#ef2f06", fontWeight: 800, my: 0.3 }}>
                    {p.sale}
                  </Typography>
                  <CardActions sx={{ pl: 0, pt: 0.5 }}>
                    <Button
                      variant="contained"
                      color="warning"
                      size="small"
                ...
                <Grid item xs={5} sx={{ position: "relative", minHeight: 120 }}>
                  <CardMedia component="img" height="100%" image={p.image} alt={p.title} sx={{ objectFit: "contain", p: 1 }} />
                </Grid>
              </Grid>
