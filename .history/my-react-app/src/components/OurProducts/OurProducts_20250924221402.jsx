        ...
        {visible.map((p) => (
          <Grid key={p.id} item xs={12} sm={6} md={3} sx={{ display: 'flex' }}>
            <Card sx={{ p: 1.5, border: '1px solid #eee', borderRadius: 2, height: 380, display: 'flex', flexDirection: 'column', width: '100%' }}>
              ...
              <CardActions sx={{ justifyContent: 'center', pt: 0, mt: 'auto' }}>
                <Button variant="contained" color="warning" size="small" sx={{ minWidth: 120 }} onClick={() => dispatch(addToCart({ id: p.id, name: p.name, price: p.price }))}>Add To Cart</Button>
              </CardActions>
            </Card>
          </Grid>
        ))}
