        ...
        {visible.map((p) => (
          <Grid key={p.id} item xs={12} sm={6} md={3} sx={{ display: 'flex' }}>
            <Card sx={{ p: 1.5, border: '1px solid #eee', borderRadius: 2, height: 380, display: 'flex', flexDirection: 'column', width: '100%' }}>
              ...
              <CardContent sx={{ textAlign: 'center', flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: 120 }}>
                <Box>
                  <Typography variant="caption" sx={{ color: '#8a8a8a' }}>{p.category}</Typography>
                  <Typography variant="subtitle1" sx={{ fontWeight: 700, whiteSpace: 'pre-line', minHeight: 48, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{p.name}</Typography>
                </Box>
