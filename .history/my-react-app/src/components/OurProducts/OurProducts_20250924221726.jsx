    ...
    <Box component="section" sx={{ width: "100%", px: { xs: 2, md: 3 }, py: 3 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2, gap: 2, maxWidth: 1200, mx: 'auto', px: { xs: 1, md: 2 } }}>
        ...
        <Tabs
          ...
          sx={{ '& .MuiTab-root': { textTransform: 'none', borderRadius: 999, minHeight: 36, fontSize: '0.9rem', px: 2 }, '& .Mui-selected': { bgcolor: '#ffefdd' } }}
        >
      ...
      <Grid container spacing={2} alignItems="stretch" sx={{ maxWidth: 1200, mx: 'auto' }}>
        {visible.map((p) => (
          <Grid key={p.id} item xs={12} sm={6} md={3} sx={{ display: 'flex' }}>
            <Card sx={{ p: 1.5, border: '1px solid #eee', borderRadius: 2, height: 380, display: 'flex', flexDirection: 'column', width: '100%' }}>
              <Box sx={{ position: 'relative', height: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: '#fafafa' }}>
                {p.badge && (
                  <Box sx={{ position: 'absolute', top: 8, right: 8, bgcolor: p.badge === 'Sale' ? '#ef2f06' : '#f28900', color: '#fff', px: 1.2, py: 0.4, borderRadius: 999, fontSize: 12, fontWeight: 700, zIndex: 1 }}>
                    {p.badge}
                  </Box>
                )}
                <CardMedia component="img" image={p.image} alt={p.name} sx={{ height: 160, width: 160, objectFit: 'cover', borderRadius: 1 }} />
              </Box>
              <CardContent sx={{ textAlign: 'center', flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: 120, padding: '16px 16px 8px 16px' }}>
                <Box sx={{ height: 60, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  <Typography variant="caption" sx={{ color: '#8a8a8a', lineHeight: 1.2 }}>{p.category}</Typography>
                  <Typography variant="subtitle1" sx={{ fontWeight: 700, whiteSpace: 'pre-line', fontSize: '0.9rem', lineHeight: 1.3, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{p.name}</Typography>
                </Box>
                <Box sx={{ display: 'flex', justifyContent: 'center', gap: 1, alignItems: 'baseline', height: 30 }}>
              ...
              <CardActions sx={{ justifyContent: 'center', padding: '8px 16px 16px 16px', height: 60, display: 'flex', alignItems: 'center' }}>
                <Button variant="contained" color="warning" size="small" sx={{ minWidth: 120, height: 36 }} onClick={() => dispatch(addToCart({ id: p.id, name: p.name, price: p.price }))}>Add To Cart</Button>
              </CardActions>
            </Card>
          </Grid>
        ))}
